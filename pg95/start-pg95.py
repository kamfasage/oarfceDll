...
import os, sys, util, subprocess, json
...
if not os.path.isdir(datadir):
  rc=os.system('python -u ' + homedir + os.sep + 'init-' + pgver + '.py')
  if rc == 0:
    rc=os.system('python -u ' + homedir + os.sep + 'config-' + pgver + '.py')
  else:
    sys.exit(rc)
  datadir = util.get_column('datadir', pgver)
  first_time="yes"
...
cmd = "python " + homedir + os.sep  + "run-pgctl.py"
