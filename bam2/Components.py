...
class Components(ComponentAction):
    ...
    @inlineCallbacks
    def getRelNotes(self, comp):
        """
        Method to get release notes of component.
        :return: It yields string.
        """
        rel_notes=unicode(str(util.get_relnotes (comp)),sys.getdefaultencoding(),errors='ignore').strip()
        yield self.session.publish('com.bigsql.onGetRelNotes', rel_notes)
    ...
    def selectedLog(self,logdir):
        ...
        if logdir == None:
            ...
            for _li in _lines:
                line = unicode(str(_li),sys.getdefaultencoding(),errors='ignore').strip()
    ...
    def liveLog(self,logdir):
        ...
        if logdir == None:
            ...
            if ln:
                for log_line in ln:
                    line = unicode(str(log_line),sys.getdefaultencoding(),errors='ignore').strip()
                    yield self.session.publish('com.bigsql.log', line)
