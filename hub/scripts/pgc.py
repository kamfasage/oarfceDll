...
if sys.version_info >= (3, 0):
  util.exit_message("Currently we run best on Python 2.7", 1)
...
def retrieve_comp(p_base_name, component_name=None):
  ...
  my_logger.info(msg)
  if not util.http_get_file(isJSON, bz2_file, REPO, conf_cache, True, msg, component_name):
    return (False)
...
def get_list():
  ...
  if isExtensions:
    ...
    available_category_conditions = " AND p.category = 2 "
    if p_comp != "all":
      ext_component = " AND v.parent = '" + p_comp + "' "

  installed = \
    "SELECT p.category, g.description as category_desc, c.component, c.version, c.port, c.status, \n" + \
    "       r.stage, (select is_current from versions where c.component = component AND c.version = version), \n" + \
    " c.datadir, p.short_desc, v.parent, v.release_date, c.install_dt \n" + \
    "  FROM components c, releases r, projects p, categories g, versions v \n" + \
    " WHERE c.component = r.component AND r.project = p.project AND v.component = c.component \n" + \
    "   AND p.category = g.category AND v.version = c.version "  + \
  ...
  available = \
    ...
    "       r.stage, v.is_current, '', p.short_desc, v.parent, v.release_date, '' \n" + \
...
def wildcard_component(p_comp):
  data = []
  sql = "SELECT component FROM releases WHERE component LIKE '" + p_comp + "%'"
  try:
    c = connL.cursor()
    c.execute(sql)
    data = c.fetchall()
...
try:
  ...
  if (p_mode in ('register', 'unregister')):
    ...
    if len(args) != 4:
      print "ERROR: must be exactly two parms"
      util.exit_message(bad_reg_msg, 1)

    if args[2] not in ('HOST', 'PG'):
      print "ERROR: first parm must be HOST or PG"
      util.exit_message(bad_reg_msg, 1)

    host_array = args[3].split(':')
    if p_mode == 'unregister' and len(host_array) != 1:
      print "ERROR: second parm must be just 'host' for UNREGISTER command"
      util.exit_message(bad_reg_msg, 1)
    if p_mode == 'register' and len(host_array) != 4:
      print "ERROR: second parm must be 'host:pgc_home:user:passwd' for REGISTER command"
      util.exit_message(bad_reg_msg, 1)

    if p_mode == 'register':
      rc=0
      if util.check_remote_host(args[3]):
        rc = util.register(args[2], args[3])
    else:
      rc = util.unregister(args[2], args[3])
    ...
    if(p_comp=="all" and info_arg==0):
      ...
      try:
        ...
        sql = "SELECT c.category, c.description, p.project, r.component, v.version, v.platform, \n" + \
              "       p.homepage_url, r.doc_url, v.rel_notes_url, v.is_current, \n" + \
        ...
        for row in data:
          ...
          if isRELNOTES:
            compDict['relnotes'] = util.get_relnotes(comp)
          else:
            compDict['relnotes'] = ""
