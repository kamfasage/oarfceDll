...
def top(display=True, isJson=False):
  ...
  for pp in processes:
    ...
    ctime = "%s:%s.%s" % (ctime.seconds // 60 % 60,
                          ...
                          str(ctime.microseconds)[:2].ljust(2,'0'))
...
def info(p_json, p_home, p_repo, print_flag=True):
  ...
  if this_uname == "Darwin":
    system_memory_in_bytes = int(commands.getoutput("/usr/sbin/sysctl hw.memsize | awk '{print $2}'"))
    system_memory_in_gb = system_memory_in_bytes / 1024 / 1024 / 1024
    system_cpu_cores = int(commands.getoutput("/usr/sbin/sysctl hw.physicalcpu | awk '{print $2}'"))
    cpu_model=commands.getoutput("/usr/sbin/sysctl -n machdep.cpu.brand_string")
    ...
    system_memory_in_mb=int(commands.getoutput("free -m | awk '/Mem:/ {print $2}'"))
    system_memory_in_gb = system_memory_in_mb / 1024.0
    system_cpu_cores=int(commands.getoutput("egrep -c 'processor([[:space:]]+):.*' /proc/cpuinfo"))
    cpu_model=commands.getoutput("grep 'model name' /proc/cpuinfo | head -1 | cut -d':' -f2")
  ...
  print(style_start + "# Last Update UTC: " + style_end + str(last_update_utc))
  if interval:
    print(style_start + "# Update Interval: " + style_end + str(interval))
    print(style_start + "# Next Update UTC: " + style_end + str(next_update_utc))
  print(style_start + ("#" * 75) + style_end)
